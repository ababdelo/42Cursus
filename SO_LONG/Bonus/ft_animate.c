...
/*   Updated: 2023/03/06 22:54:53 by ababdelo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	animate_collectibles(t_data *data, int index)
{
	...
	while (++index < data->collect_cntr)
	{
		...
		if (++i == 0)
			put_collect_up(data, j, index);
		else if (i == 1)
			put_collect_down(data, j, index);
		else
			i = -1;
	}
	frame = 0;
...
void	put_collect_up(t_data *data, int j, int index)
{
	if (data->buff[j] == 'C')
	{
		mlx_put_image_to_window(data->mlx, data->mlx_win, data->dirt_,
...
void	put_collect_down(t_data *data, int j, int index)
{
	if (data->buff[j] == 'C')
	{
		mlx_put_image_to_window(data->mlx, data->mlx_win, data->dirt_,
