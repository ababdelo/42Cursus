...
/*   Updated: 2023/03/05 15:00:16 by ababdelo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_player_collectibles_textures(t_data *data)
{
	...
	if (!data->collect_)
		print_msg("couldn't found Collectible texture !\n", 1);
	data->player_ = mlx_xpm_file_to_image(data->mlx,
			"./textures/Player/centered_face.xpm", &width, &height);
	if (!data->player_)
		print_msg("couldn't found centered player face texture !\n", 1);
	return ;
}
